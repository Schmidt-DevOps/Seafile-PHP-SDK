...
try {
    ...
    $logger->log(Logger::INFO, "#################### Sleeping 1s before deleting the account... zzzzzz....");
    sleep(1);
...
//
    ...
    $fileToUpload = "/home/re/helloworld.txt";
    $dir = '/Test234'; // directory in the library to save the file in
    $response = $fileResource->upload($lib, $fileToUpload, $dir);
    $uploadedFileId = (string)$response->getBody();

    $logger->log(Logger::INFO, "#################### Uploaded file ID " . $uploadedFileId);
} catch (\Exception $e) {
